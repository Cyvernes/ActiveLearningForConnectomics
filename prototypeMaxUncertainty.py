...
def getLabel(
    seed, GT_mask
):  # seed should be given in the SAM predict format and not in the image format
...
if __name__ == "__main__":
    ...
    # find first seed
    se_masks = sorted(
        mask_generator.generate(image), key=lambda mask: mask["predicted_iou"]
    )
    nb_seeds = len(se_masks)
    se_mask = se_masks.pop(-1)[
        "segmentation"
    ]  # pop(-2) to have a mitochondrion and pop(-1) to have the background
    ...
    while i < nb_seeds != 0:
        ...
        if look_for_first_GT_mitochondria:
            potential_seeds = sorted(
                potential_seeds, key=lambda seed: getEvidence(seed, evidence)
            )
        ...
        if SAVE_INTERMEDIATE_RESULTS:
            ...
            axs[0, 0].scatter(
                [new_seed[0]],
                [new_seed[1]],
                color="green" if getLabel(new_seed, GT_mask) else "red",
            )
