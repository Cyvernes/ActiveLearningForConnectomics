...
def setup_learner(config, sam):
    learner_types = {
        "Active Learning": ActiveLearningSAM,
        "Pseudo Active Learning": PseudoActiveLearningSAM,
        "FPFN": FPFNLearner,
    ...
    return LearnerClass(
        sam,
        getattr(strategy_selectors, config["STRATEGY_SELECTOR"]),
        [
            getattr(learning_strategies, strategy)
            for strategy in config["LEARNING_STRATEGIES"]
        ],
        getattr(first_seeds_selector, config["FIRST_SEEDS_SELECTOR"]),
        [
            getattr(next_seeds_strategies, strategy)
            for strategy in config["SEED_SELECTION_STRATEGIES"]
        ],
...
def find_first_seeds(learner, image, GT_mask, idx, config):
    ...
    if config["SAVE_FIRST_SEED"]:
        plotAndSaveImageWithFirstSeed(
            config["FOLDER_FOR_INTERMEDIATE_RESULTS"], image, first_seeds, idx
        )

    if config["SAVE_IMAGE_WITH_GT"]:
        plotAndSaveImageWithGT(
            config["FOLDER_FOR_INTERMEDIATE_RESULTS"], image, GT_mask, idx
        )
...
def run_learning_process(learner, GT_mask, first_seeds, nb_seeds, image, idx, config):
    ...
    budget = (
        config["training_parameters"]["ANNOTATION_BUDGET"]
        if config["training_parameters"]["USE_BUDGET"]
        else nb_seeds
    )
    nb_annotations = 0
    count = 0
    while nb_annotations < budget:
        ...
        if config["plot_parameters"]["SAVE_INTERMEDIATE_RESULTS"]:
            plotAndSaveIntermediateResults(
                config["plot_parameters"]["FOLDER_FOR_INTERMEDIATE_RESULTS"],
                learner,
                next_seeds,
                image,
                GT_mask,
        ...
        if (
            nb_annotations == budget - 1
            and config["plot_parameters"]["SAVE_FINAL_RESULT"]
        ):
            plotAndSaveIntermediateResults(
                config["plot_parameters"]["FOLDER_FOR_INTERMEDIATE_RESULTS"],
                learner,
...
def run_model_single_image(
    learner, image_link, mask_link, idx, aggregated_results, config
):
    image = cv2.imread(image_link)
    GT_mask = np.any(cv2.imread(mask_link) != [0, 0, 0], axis=-1)

    first_seeds, nb_seeds = find_first_seeds(
        learner, image, GT_mask, idx, config["plot_parameters"]
    )
    NBs, IoUs, FPs, FNs, percentiles = run_learning_process(
        learner, GT_mask, first_seeds, nb_seeds, image, idx, config
    )
    plots_for_single_image(
        NBs, IoUs, FPs, FNs, percentiles, idx, config["plot_parameters"]
    )
...
def run_model_dataset(learner, images_links, masks_links, config):
    ...
    total_number_images = int(
        config["training_parameters"]["TRAIN_RATIO"] * len(images_links)
    )
    for idx in range(total_number_images):
        print(
            "-------------------------------------", idx + 1, "/", total_number_images
        )
        if (
            run_model_single_image(
                learner,
                images_links[idx],
                masks_links[idx],
                idx,
    ...
    if config["plot_parameters"]["SAVE_AGGREGATED_RESULTS"]:
        plotandSaveAggregatedResults(
            config["plot_parameters"]["FOLDER_FOR_FINAL_RESULTS"], aggregated_results
        )
