...
def ArgmaxEvInSESeeds(learner) -> Tuple[int, int]:
...
def ArgmaxDist(learner)  -> Tuple[int, int]:
    p_thresh = 0.8
...
def ArgmaxUncertainty(learner)  -> Tuple[int, int]:
...
def ArgmaxUncertaintyPathDist(learner)  -> Tuple[int, int]:
    p_thresh = 0.95
    evidence_thresh = np.log(p_thresh/ (1 - p_thresh))
