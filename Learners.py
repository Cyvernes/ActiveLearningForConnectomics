...
class ActiveLearningSAM:
    ...
    def findFirstSeeds(self) -> Tuple[List[Tuple[int, int]], int, np.ndarray]:#masks from Segment Everything are in the same format as the image
        self.SE_masks = sorted(self.mask_generator.generate(self.image), key = lambda mask: mask['predicted_iou'])#masks from segement every thing
        self.SE_Seeds = removeTooCloseSeeds([swap(findVisualCenter(mask['segmentation'])) for mask in self.SE_masks])#SE_seeds are saved in the input format
    ...
    def findNextSeeds(self) -> list:
        ...
        next_seeds = self.seeds_selection_strategies[self.current_strategy_idx % len(self.seeds_selection_strategies)](self)
        return(next_seeds)
...
class FPFNLearner(ActiveLearningSAM):
    ...
    """
    def __init__(self, model, strategy_selector, learning_strategies, seeds_selection_strategies, uncertainty_fn, filtering_fn, filtering_aux_fn, image=None, mask_generator=None, use_previous_logits=True):
        super().__init__(model, strategy_selector, learning_strategies, seeds_selection_strategies, uncertainty_fn, filtering_fn, filtering_aux_fn, image, mask_generator, use_previous_logits)
        self.GT_mask = None
