...
) -> bool:  # seed should be given in the input format and not in the image format
    return GT_mask[seed[1], seed[0]]


def getValueinArrFromInputFormat(
    arr: np.ndarray, seed: Tuple[int, int]
):  # seed should be given in the input format and not in the image format
...
def uncertaintyH(Ev):
    ...
    rep[where_to_compute] = -np.multiply(p[where_to_compute], np.log(p[where_to_compute])) - np.multiply(
...
def UncertaintyPathDist(uncertainty: np.ndarray, evidence: np.ndarray, thresh: float) -> np.ndarray:
    ...
    points_to_see = [
        (uncertainty[point_x, point_y], (point_x, point_y)) for point_x, point_y in zip(points_x, points_y)
    ]
        ...
        if seen_points[point[0], point[1]] == 0:
            ...
            for dx, dy in [
                ...
                if (
                    ...
                    and (0 <= point[1] + dy < w)
                    and (
                        uncertain_mask[point[0] + dx, point[1] + dy] and seen_points[point[0] + dx, point[1] + dy] == 0
                    )
                ):
                    potential_weight = (
                        np.sqrt(np.abs(dx) + np.abs(dy)) * uncertainty[point[0] + dx, point[1] + dy]
